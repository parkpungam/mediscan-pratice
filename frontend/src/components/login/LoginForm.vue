<template>
  <form class="signup-form" novalidate @submit.prevent="submit">
    <div class="form-field"><label for="login-email">이메일</label><input id="login-email" type="email" autocomplete="email" autofocus :value="email" placeholder="이메일을 입력하세요" @input="$emit('update:email', $event.target.value.replace(/\s/g, ''))" /></div>
    ...
    <label class="check-row login-keep-signed-in"><input type="checkbox" :checked="keepSignedIn" @change="$emit('update:keep-signed-in', $event.target.checked)" /><span>로그인 상태 유지</span></label>
...
<script>
export default { name:'LoginForm', emits:['update:email','update:password','update:keep-signed-in','submit'], props:{email:String,password:String,keepSignedIn:Boolean,loading:Boolean,error:String}, data(){return{showPassword:false}}, methods:{submit(){if(this.email&&this.password&&!this.loading)this.$emit('submit')}} }
</script>
